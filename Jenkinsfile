pipeline {
    ...
    environment {
        ...
        SONARQUBE_PROJECT_KEY = 'InvenTrackCore-CustomCode'
    ...
    stages {
        ...
        stage('Ejecutar Pruebas Unitarias') {
            steps {
                sh 'dotnet test InvenTrackCore.Tests/InvenTrackCore.Tests.csproj --logger trx'
            }
            post {
                failure {
                    mail to: env.NOTIFICATION_EMAIL,
        ...
        stage('Construcción de Docker') {
            ...
            steps {
                script {
                    sh "docker build -f InvenTrackCore.Api/Dockerfile -t ${DOCKER_IMAGE} ."
                }
            }
        }
